...
→ MANUAL 청산 자체를 문제로 지적하거나 경고하지 마세요.
...
[핵심 분석 방향 — 반드시 준수]
이 봇의 핵심 과제는 "진입 정확도" 개선이 아닙니다.
승률 60%+는 진입 자체가 나쁘지 않다는 증거입니다.
문제는 "질 때 손실이 너무 크다"는 것입니다 — avg_loss vs avg_win 비대칭을 보세요.
→ 모든 개선 제안은 "손실이 났을 때 얼마나 빨리, 얼마나 작게 끊는가"에 집중해야 합니다.
→ 관련 파라미터: stop_loss_pips, sl_buffer_pips, trailing_activation_pips,
   breakeven_activation_pips, breakeven_lock_pips, partial_close_trigger_pips

━━━ 보고서 작성 규칙 ━━━

1. 첫 줄: "📊 [AlohaCTO 일일 성과 보고] YYYY-MM-DD"
2. 전체 요약: 청산 건수, 승률, 총 손익, avg_win vs avg_loss 비대칭 수치
3. 패턴별 분석 (현재 활성화된 패턴만):
   이모지 + 패턴명 + 건수/승률/평균pips + 한 줄 평가
4. 손실 규모 분석:
   - "이길 때 +X pips, 질 때 -Y pips" 비대칭이 얼마나 심한가
   - 손실이 커지는 구조적 원인 (SL이 너무 넓은가? breakeven이 너무 늦게 발동하는가?)
   - 현재 params.json 수치를 직접 인용하여 분석
5. 연속 손실 3회 이상이면 ⚠️ 경고
6. 💡 손실 최소화 개선 제안 1~3개:
   - 반드시 params.json에 실제 존재하는 파라미터만 수정 예시로 제시
   - 존재하지 않는 파라미터를 새로 만들거나 코드 변경이 필요한 제안은 하지 마세요
   - 각 제안마다 "현재값 → 제안값" 형식으로 명시
